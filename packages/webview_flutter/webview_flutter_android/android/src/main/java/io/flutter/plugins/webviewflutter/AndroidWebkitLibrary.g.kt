...
abstract class AndroidWebkitLibraryPigeonProxyApiRegistrar(val binaryMessenger: BinaryMessenger) {
  ...
  /**
   * An implementation of [PigeonApiRenderProcessGoneDetail] used to add a new Dart instance of
   * `RenderProcessGoneDetail` to the Dart `InstanceManager`.
   */
  abstract fun getPigeonApiRenderProcessGoneDetail(): PigeonApiRenderProcessGoneDetail
...
private class AndroidWebkitLibraryPigeonProxyApiBaseCodec(val registrar: AndroidWebkitLibraryPigeonProxyApiRegistrar) : AndroidWebkitLibraryPigeonCodec() {
  ...
  override fun writeValue(stream: ByteArrayOutputStream, value: Any?) {
    ...
    if (value is android.webkit.WebResourceRequest) {
     ...
     else if (android.os.Build.VERSION.SDK_INT >= 26 && value is android.webkit.RenderProcessGoneDetail) {
      registrar.getPigeonApiRenderProcessGoneDetail().pigeon_newInstance(value) { }
    }
...
@Suppress("UNCHECKED_CAST")
abstract class PigeonApiRenderProcessGoneDetail(open val pigeonRegistrar: AndroidWebkitLibraryPigeonProxyApiRegistrar) {
  @androidx.annotation.RequiresApi(api = 26)
  abstract fun didCrash(pigeon_instance: android.webkit.RenderProcessGoneDetail): Boolean

  @androidx.annotation.RequiresApi(api = 26)
  abstract fun rendererPriorityAtExit(pigeon_instance: android.webkit.RenderProcessGoneDetail): Long

  @Suppress("LocalVariableName", "FunctionName")
  /** Creates a Dart instance of RenderProcessGoneDetail and attaches it to [pigeon_instanceArg]. */
  @androidx.annotation.RequiresApi(api = 26)
  fun pigeon_newInstance(pigeon_instanceArg: android.webkit.RenderProcessGoneDetail, callback: (Result<Unit>) -> Unit)
{
    if (pigeonRegistrar.ignoreCallsToDart) {
      callback(
          Result.failure(
    ...
    val pigeon_identifierArg = pigeonRegistrar.instanceManager.addHostCreatedInstance(pigeon_instanceArg)
    val didCrashArg = didCrash(pigeon_instanceArg)
    val rendererPriorityAtExitArg = rendererPriorityAtExit(pigeon_instanceArg)
    val binaryMessenger = pigeonRegistrar.binaryMessenger
    val codec = pigeonRegistrar.codec
    val channelName = "dev.flutter.pigeon.webview_flutter_android.RenderProcessGoneDetail.pigeon_newInstance"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(pigeon_identifierArg, didCrashArg, rendererPriorityAtExitArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
          callback(Result.failure(AndroidWebKitError(it[0] as String, it[1] as String, it[2] as String?)))
        } else {
...
abstract class PigeonApiWebViewClient(open val pigeonRegistrar: AndroidWebkitLibraryPigeonProxyApiRegistrar) {
  ...
  abstract fun setSynchronousReturnValueForApplicationDidHandleWebViewRenderProcessCrash(pigeon_instance: android.webkit.WebViewClient, value: Boolean)
  ...
  companion object {
    ...
    fun setUpMessageHandlers(binaryMessenger: BinaryMessenger, api: PigeonApiWebViewClient?) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.webview_flutter_android.WebViewClient.setSynchronousReturnValueForApplicationDidHandleWebViewRenderProcessCrash", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pigeon_instanceArg = args[0] as android.webkit.WebViewClient
            val valueArg = args[1] as Boolean
            val wrapped: List<Any?> = try {
              api.setSynchronousReturnValueForApplicationDidHandleWebViewRenderProcessCrash(pigeon_instanceArg, valueArg)
              listOf(null)
            } catch (exception: Throwable) {
              wrapError(exception)
            }
  ...
  fun onReceivedError(pigeon_instanceArg: android.webkit.WebViewClient, webViewArg: android.webkit.WebView, errorCodeArg: Long, descriptionArg: String, failingUrlArg: String, callback: (Result<Unit>) -> Unit)
{
  ...
  @androidx.annotation.RequiresApi(api = 26)
  fun onRenderProcessGone(pigeon_instanceArg: android.webkit.WebViewClient, webViewArg: android.webkit.WebView, dataArg: android.webkit.RenderProcessGoneDetail, callback: (Result<Boolean>) -> Unit)
{
    if (pigeonRegistrar.ignoreCallsToDart) {
      callback(
          Result.failure(
    ...
    val binaryMessenger = pigeonRegistrar.binaryMessenger
    val codec = pigeonRegistrar.codec
    val channelName = "dev.flutter.pigeon.webview_flutter_android.WebViewClient.onRenderProcessGone"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(pigeon_instanceArg, webViewArg, dataArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
          callback(Result.failure(AndroidWebKitError(it[0] as String, it[1] as String, it[2] as String?)))
        } else if (it[0] == null) {
